...
from models.three_d.gspl.render import render_rgb_from_matrices
...
def render_display(
    ...
    assert resolved_device.type == 'cuda', f"{resolved_device=}"
    camera = camera.to(resolved_device)

    scene_model.model.to(device=resolved_device)
    scene_model.device = resolved_device
    ...
    if image is None:
        image = render_rgb_from_matrices(
